...
namespace NavigationPlatform.Gateway.Messaging;

internal sealed class GatewayOutboxPublisher : BackgroundService
{
    private const string ExchangeName = "navigation.events";

    private readonly IServiceScopeFactory _scopeFactory;
    private readonly ILogger<GatewayOutboxPublisher> _logger;

    public GatewayOutboxPublisher(
        IServiceScopeFactory scopeFactory,
        ILogger<GatewayOutboxPublisher> logger)
    {
    ...
    {
        _logger.LogInformation("GatewayOutboxPublisher starting");
        ...
        while (!stoppingToken.IsCancellationRequested)
        {
            ...
            {
                _logger.LogError(ex, "GatewayOutboxPublisher failed; retrying soon");
                await Task.Delay(TimeSpan.FromSeconds(5), stoppingToken);
            }
        }

        _logger.LogInformation("GatewayOutboxPublisher stopped");
    }
}
